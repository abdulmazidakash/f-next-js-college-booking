...
export default function MyCollege() {
  ...
  useEffect(() => {
    async function fetchUserAdmission() {
      if (status === "authenticated" && session?.user?.email) {
        try {
          // You'll need a backend API to fetch user-specific admissions
          ...
          if (data && data.length > 0) {
            // Assuming a user has one primary admission for simplicity.
            // If multiple, you might show a list and let them pick one to review.
            ...
            setUserAdmission(null); // No admission found for user
          }
        } catch (error) {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const reviewData = {
        ...review,
        userId: session.user.id, // Or session.user.email for uniqueness
        userName: session.user.name || session.user.email,
        // Ensure collegeId and collegeName are populated from userAdmission state
      };
      ...
      setReview({ text: "", rating: 0, collegeId: userAdmission?.collegeId || "", collegeName: userAdmission?.collegeName || "" }); // Reset review form
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-2xl">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        <p className="text-xl font-semibold mb-3">Welcome, {session.user.name}!</p>
        {userAdmission ? (
          <div>
          ...
          <p className="text-gray-600">You haven't submitted an admission yet. Go to the <Link href="/admission" className="text-blue-600 hover:underline">Admission page</Link> to apply!</p>
        )}
      </div>

      {userAdmission && ( // Only show review form if admission exists
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <form onSubmit={handleReviewSubmit} className="space-y-4">
            ...
            <button
              ...
              className={`w-full bg-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700 transition-colors ${submittingReview ? "opacity-50 cursor-not-allowed" : ""}`}
            >
